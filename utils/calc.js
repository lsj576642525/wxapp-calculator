 ...
 */
function tryDelete() {
  if (curState == STATE.SECOND_DOT || curState == STATE.SECOND_UNDOT) {
    if (opNum2.length > 0) {
...
function tryCalc() {
  ...
  switch (op) {
    case "+":
      if (curResult !== 0) {
        curResult = customMath.plus(curResult, n2);
      } else {
        curResult = customMath.plus(n1, n2);
      }
      break;
    case "-":
      if (curResult !== 0) {
        curResult = customMath.minus(curResult, n2);
      } else {
        curResult = customMath.minus(n1, n2);
      }
      break;
    case "x":
      if (curResult !== 0) {
        curResult = customMath.times(curResult, n2);
      } else {
        curResult = customMath.times(n1, n2);
      }
      break;
    case "/":
      if (n2 == 0) {
        ...
        if (curResult !== 0) {
          curResult = customMath.divide(curResult, n2);
        } else {
          curResult = customMath.divide(n1, n2);
        }
      }
      break;
    case "%":
  ...
  switch (curState) {
    ...
    case STATE.FIRST_DOT:
      ...
      if (isNumber(code)) {
        ...
        tryDelete();
        if (opNum1.indexOf(".") < 0) curState = STATE.FIRST_UNDOT;
      } else if (isOperator(code)) {
        curState = STATE.SECOND_UNDOT;
        op = code;
        opNum2 = "";
    ...
    case STATE.SECOND_DOT:
      if (isNumber(code)) {
        ...
        tryDelete();
        if (opNum2.indexOf(".") < 0) curState = STATE.SECOND_UNDOT;
        displayNum = opNum2;
      } else if (isOperator(code)) {
        if (opNum2 != "") {
          //直接计算
